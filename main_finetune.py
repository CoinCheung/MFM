...
parser.add_argument('--save-path', metavar='DIR', default='./res',
                    help='path to dataset (default: ./res)')
parser.add_argument('--label-smoothing', default=0.0, type=float, help='label smoothing (default: 0.0)', dest='label_smoothing'
    )
parser.add_argument('--mixup-alpha', default=0.1, type=float, help='mixup alpha (default: 0.0)')
...
def main():
    ...
    if not osp.exists(args.save_path): os.makedirs(args.save_path)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.ckpt:
        print(f"=> load pre-trained weights from ckpt: {args.ckpt}")
    ...
    ## TODO: randaug的6/0.5的6是mag吗，0.5是啥
    ## 使用torchvision的randaug会掉点吗，不带0.5的
    train_dataset = datasets.ImageFolder(
        traindir,
        transforms.Compose([
            transforms.RandomResizedCrop(224),
            transforms.RandomHorizontalFlip(),
            rand_augment_transform(config_str='rand-m6-mstd0.5'),
            transforms.ToTensor(),
            normalize,
        ]))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            filename = osp.join(args.save_path, filename)
            save_checkpoint({
                'epoch': epoch + 1,
                'arch': args.arch,
